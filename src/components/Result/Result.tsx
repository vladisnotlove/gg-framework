  ...
  return (
    <Modal className={classNames("gg-result", className)} open={open}>
      <div className="gg-result__window">
        <div className="gg-result__pattern"></div>
        <div
        ...
        <div className="gg-result__body">
          <div className="gg-result__title">
            {title ?? translate("YOU WIN!")}
          </div>
          <div className="gg-result__prize">
            <div className="gg-result__prize-count">{prizeCount ?? 0}</div>
            <div className="gg-result__prize-currency">G chips</div>
          </div>
          {bodyAfter}
        </div>
        <Button
          {...submitBtnProps}
          className={classNames("gg-result__ok-btn", submitBtnProps?.className)}
          onClick={() => {
            if (submitBtnProps?.onClick) submitBtnProps.onClick();
            if (onClose) onClose();
          }}
        >
          {submitBtnContent ?? translate("AWESOME")}
        </Button>
      </div>
    </Modal>
  );
